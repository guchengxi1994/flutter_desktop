...
class SevenSegmentTextPainter {
  List<Path> segments =
      []; // starting from top, going clockwise, center segment at the end
  ...
  SevenSegmentTextPainter(
      {required this.height, required this.digits, required this.number}) {
  ...
  Size layout() {
    return Size(
        size.width * digits * letterSpacing -
            (size.width * (letterSpacing - 1)),
        size.height);
  }
  ...
  void drawDigit(Canvas canvas, Offset position, int number) {
    ...
    for (int i = 0; i < segments.length; i++) {
      canvas.drawPath(segments[i].shift(position),
          segmentsToLight[number][i] ? activeColor : inactiveColor);
    }
  }
