...
class MinesweeperGameCubit extends Cubit<MinesweeperGameState> {
  ...
  factory MinesweeperGameCubit(MinesweeperSettings settings) {
    ...
    return MinesweeperGameCubit._(field, settings.bombs, const Ticker());
  }
  ...
  void openCell(IntPos pos) {
    ...
    if (state is MinesweeperGameGoing) {
      ...
      emit(state.copyWith(
        field: board,
        modifier: const MinefieldHighlightNone(),
  ...
  void unPressCell() {
    ...
    if (state is MinesweeperGameGoing) {
      emit(state.copyWith(modifier: const MinefieldHighlightNone()));
    }
...
List<List<MinesweeperCell>> generateField(int w, int h) {
  return List.generate(w, (_) => List.filled(h, const MinesweeperCell()));
}
